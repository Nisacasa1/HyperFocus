...
### 📋 Daily Planning
Lives inside **The Day** — one place to decide, one place to review.
- Set your **3 Most Important Tasks** (MITs) when you open the day
...
- **One column, one activity at a time.** The day is a strict partition — nothing ever overlaps. Log something during a session and it *splits* the session: the part before the interruption, the interruption, then the session resuming (↳) after it. That is what actually happened, and it is what the calendar shows.
- **The tracker and your sessions complement each other, they never mix.** A focus session is never written into the tracker log — it is drawn on the calendar straight from your focus history, and whatever you log simply carves into it. The summary counts **Tracked** and **Focused** separately.
