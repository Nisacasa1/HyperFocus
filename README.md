...
- **Overlapping blocks cascade, like a calendar.** Anything sharing the same stretch of time is indented and drawn on top of what it overlaps, still running to the right edge, so the block underneath keeps a visible strip. Hover — or tap, on a phone — brings a buried block to the front. A block alone on its stretch takes the full width.
- **The tracker and your sessions complement each other, they never mix.** A focus session is never written into the tracker log — it is drawn on the calendar straight from your focus history, and sits at the back of the stack as the context your logged time falls inside. The summary counts **Tracked** and **Focused** separately.
